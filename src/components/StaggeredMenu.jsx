...
export const StaggeredMenu = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      console.log('Setting initial positions, offscreen:', offscreen, 'elements:', [panel, ...preLayers]);
      // Remove initial hidden classes and set GSAP control
      panel.classList.remove('sm-initial-hidden');
      preContainer?.classList.remove('sm-initial-hidden');
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    console.log(`Animating panel from ${panelStart}% to 0%`);
    tl.fromTo(
      panel,
      { xPercent: panelStart },
  ...
  const playOpen = useCallback(() => {
    console.log('playOpen called, busy:', busyRef.current);
    if (busyRef.current) return;
    busyRef.current = true;
    const tl = buildOpenTimeline();
    console.log('Timeline built:', tl);
    if (tl) {
      ...
      console.log('No timeline created');
      busyRef.current = false;
    }
  }, [buildOpenTimeline]);
  ...
  const toggleMenu = useCallback(() => {
    console.log('Toggle menu clicked!', { currentOpen: openRef.current });
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers sm-initial-hidden" aria-hidden="true">
        {(() => {
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel sm-initial-hidden" aria-hidden={!open}>
